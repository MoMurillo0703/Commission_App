...
- Individual and Team reports are **current calculated unpaid earnings**. They start from posted commissions in the selected Paid Month, resolve the allocation and Team membership that cover that Paid Month, settle with `settleAllocation()`, and aggregate recipient leaves. Stored `commission_payouts` are historical calculation/audit artifacts and do not control whether a commission appears. Running a report never writes financial records.
- If no explicit allocation covers Group + LOB + Paid Month, current earnings default to **Agency 100%**. That is a valid $0 result for other recipients. Do not infer compensation from assignment, Group name, a later configuration, or source period. Saving a Group/LOB that Mo left at this default persists an explicit Agency 100% allocation so later history can distinguish “never configured” from “intentionally Agency 100%.”
- A future-effective allocation never applies backward. A malformed covering allocation or invalid Team membership is **REVIEW REQUIRED** and is not silently omitted or calculated.
